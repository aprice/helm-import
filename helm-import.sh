...
echo " "
${HELM_BIN} template "${release_name}" "${chart}" ${@} | kubectl annotate -f- "meta.helm.sh/release-name=${release_name}" "meta.helm.sh/release-namespace=${HELM_NAMESPACE}" --overwrite -n "${HELM_NAMESPACE}" || true
echo " "

echo "Adding labels to all resources found by running template command ..."
echo ${HELM_BIN} template "${release_name}" "${chart}" ${@}
echo " "
${HELM_BIN} template "${release_name}" "${chart}" ${@} | kubectl label -f- "app.kubernetes.io/managed-by=Helm" --overwrite -n "${HELM_NAMESPACE}" || true
echo " "
